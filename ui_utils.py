    ...
    with col2:
        ...
        if improvements:
            for improvement in improvements:
                st.markdown(
                    f"""<div style="background-color: #B23434; color: white; padding: 12px; 
                    border-radius: 6px; margin-bottom: 10px; font-weight: 500;">
                    ⚠️ {improvement}</div>""", 
                    unsafe_allow_html=True
                )
            ...
            st.markdown(
                """<div style="background-color: #1F7A63; color: white; padding: 12px; 
...
def display_matching_skills(skills, job_description):
    ...
    if missing_skills:
        ...
        for skill in missing_skills[:5]:  # Show top 5 missing skills
            missing_html += f"""<div style="background-color: #B23434; color: white; 
            padding: 8px 12px; border-radius: 20px; font-weight: 500; margin-bottom: 8px;">
            ⚠️ {skill.title()}</div>"""
